...
class DataProvider
{
    ...
    public static function provideMapData() : array
    {
        return [
            ...
            '16-bit map #1' => [array_fill(1, 16, 0x05), "\xde\x00\x10".array_reduce(range(1, 16), static function ($r, $i) { return $r .= pack('C', $i)."\x05"; })],
            '16-bit map #2' => [array_fill(1, 65535, 0x05), "\xde\xff\xff".array_reduce(range(1, 127), static function ($r, $i) { return $r .= pack('C', $i)."\x05"; }).array_reduce(range(128, 255), static function ($r, $i) { return $r .= "\xcc".pack('C', $i)."\x05"; }).array_reduce(range(256, 65535), static function ($r, $i) { return $r .= "\xcd".pack('n', $i)."\x05"; })],
            '32-bit map' => [array_fill(1, 65536, 0x05), "\xdf\x00\x01\x00\x00".array_reduce(range(1, 127), static function ($r, $i) { return $r .= pack('C', $i)."\x05"; }).array_reduce(range(128, 255), static function ($r, $i) { return $r .= "\xcc".pack('C', $i)."\x05"; }).array_reduce(range(256, 65535), static function ($r, $i) { return $r .= "\xcd".pack('n', $i)."\x05"; })."\xce".pack('N', 65536)."\x05"],
